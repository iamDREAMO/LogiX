    ...
    def updated():
        ...
        for loop in r:
            ...
            def updatedata2():
                mydb = sqlite3.connect('mydemo.db')
                my_cursor = mydb.cursor()
                my_cursor.execute("update ins set UNAME ='"+s3.get()+"',UPHY ='"+s4.get()+"',
                                UCHE='"+s5.get()+"',UMATHS ='"+s6.get()+"' WHERE URNO='"+s2.get()+"' ")
                mydb.commit()
                mydb.close()
...
def mylogin():
    ...
    myl1.place(x=200,y=100)mye1 = Entry(myf2, font = ('Calibri',15), textvariable= d)
