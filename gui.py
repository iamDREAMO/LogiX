...
def create_insert_tab(ntb):
    ...
    Button(f4, font=('Cambria', 15), text='Insert Data', fg='White', bg="#9F00FF",
           command=insert_data).place(x=300, y=320, width=150, height=40, anchor= CENTER)
...
def create_search_tab(ntb):
    ...
    Label(f6, font=('Cambria', 12), text='Enter Roll No.', bg="#F0F8FF", 
        ...
        if result:
            ...
            for i, text in enumerate(labels):
                Label(f6, text=f"{text}: {result[i+1]}", 
                      ...
                      fg="#9F00FF").place(x=300, y=120 + 50 * i, anchor= CENTER)
        else:
            messagebox.showinfo('Not Found', 'Roll No. not found')
    
    Button(f6, text='Search', font=('Cambria', 15), fg="#F0F8FF", bg="#9F00FF",
          command=search_student).place(x=360, y=55, width=100, height=30)
    ...
    def retrieve_student():
        ...
        for i, (label, var) in enumerate(fields):
            Label(f7, text=f'{label}:', font=('Cambria', 13, 'bold'), bg="#F0F8FF", 
                  fg="#9F00FF").place(x=150, y=100 + 50 * i)
            Entry(f7, bg="#F1F0EE", font=('Cambria', 13, 'bold'), 
                  textvariable=var).place(x=300, y=120 + 50 * i, width= 100, anchor= CENTER)
        ...
        Button(f7, text='Update', font=('Cambria', 15), fg="#F0F8FF", bg="#9F00FF",
               command=update_data).place(x=300, y=330, width=110, height=40, anchor= CENTER)
    
    Button(f7, text='Retrieve', font=('Cambria', 15), fg="#F0F8FF", bg="#9F00FF",
           command=retrieve_student).place(x=360, y=55, width=110, height=30)
...
def create_delete_tab(ntb):
    ...
    Label(f8, font=('Cambria', 12), text='Enter Roll No.', bg="#F0F8FF", 
          fg="#9F00FF").place(x=120, y=60, width=120)
    Entry(f8, font=('Cambria', 12), bg="#F1F0EE",
          textvariable=roll_no).place(x=250, y=55, width=100, height= 30)
    ...
    Button(f8, text='Delete', font=('Cambria', 15), fg="#F0F8FF", bg="#9F00FF",
           command=delete_student).place(x=360, y=55, width=100, height=40)
...
def create_login_screen(root):
    ...
    Button(f2, text='Registration', font=('Cambria', 15), fg="White", bg="#9F00FF",
           command=lambda: create_register_screen(root)).place(x=425, y=350, width=150, height=40)
...
def create_register_screen(root):
    ...
    Label(f3, text='Enter Name:', bg="#F0F8FF", font=('Cambria', 12),
          fg="#9F00FF").place(x=100, y=100)
    Entry(f3, bg= "#F1F0EE", font=('Cambria', 12), 
          textvariable=username).place(x=250, y=100, width=250, height=35)
    
    Label(f3, text='Enter Password:', bg="#F0F8FF", font=('Cambria', 12),
          fg="#9F00FF").place(x=100, y=150)
    Entry(f3, bg= "#F1F0EE", font=('Cambria', 12), 
          textvariable=password, show='*').place(x=250, y=150, width=250, height=35)
